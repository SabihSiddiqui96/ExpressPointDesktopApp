...
async function signOutRapidly(page: Page): Promise<void> {
  await clickMenuButton(page);
  await page.locator('ion-menu ion-item, ion-item, ion-button')
    .filter({ hasText: /Sign Out|Logout|Log Out/i })
    .first()
    .click({ timeout: 10_000 });
  if (await page.locator('body').innerText().then(text => /session open|close it before exiting/i.test(text)).catch(() => false)) {
    await clickOpenSessionContinue(page);
  }
  await waitForText(page, /Use PrimeroEdge Login|Username|Password|Device Information/i, 30_000);
}

async function clickVisibleTextByCoordinates(page: Page, label: RegExp): Promise<void> {
  const coords = await page.evaluate(({ source, flags }) => {
    const regex = new RegExp(source, flags);
    const candidate = Array.from(document.querySelectorAll<HTMLElement>('button, ion-button, .alert-button, div, span'))
      .filter(el => {
        const visible = !!(el.offsetWidth || el.offsetHeight || el.getClientRects().length);
        const rect = el.getBoundingClientRect();
        const text = (el.innerText ?? el.textContent ?? '').trim();
        return visible && rect.width > 10 && rect.height > 10 && regex.test(text);
      })
      .sort((a, b) => {
        const aRect = a.getBoundingClientRect();
        const bRect = b.getBoundingClientRect();
        const aIsButton = a.matches('button, ion-button, .alert-button') ? 0 : 1;
        const bIsButton = b.matches('button, ion-button, .alert-button') ? 0 : 1;
        return aIsButton - bIsButton
  ...
  expect(coords, `visible text ${label} should be clickable`).not.toBeNull();
  await page.mouse.click(coords!.x, coords!.y);
}

async function navigateToDeviceInformation(page: Page): Promise<void> {
  const clicked = await page.evaluate(() => {
    const visible = (el: HTMLElement) => !!(el.offsetWidth || el.offsetHeight || el.getClientRects().length);
    const viewportArea = window.innerWidth * window.innerHeight;
    const candidates = Array.from(document.querySelectorAll<HTMLElement>('ion-button, button, ion-item, a, ion-icon, img'))
      .filter(el => {
        if (!visible(el)) return false;
        const rect = el.getBoundingClientRect();
        const label = [
        ...
        return /Device Information|device|info|settings|cog|help|ellipsis/i.test(label)
          || (rect.left > window.innerWidth * 0.75 && rect.top > window.innerHeight * 0.7 && rect.width * rect.height < viewportArea * 0.08);
      })
      .sort((a, b) => {
        const aRect = a.getBoundingClientRect();
        const bRect = b.getBoundingClientRect();
        return (bRect.left + bRect.top) - (aRect.left + aRect.top);
      });
    const candidate = candidates[0];
    (candidate?.closest('ion-button, button, ion-item, a') as HTMLElement | null ?? candidate)?.click();
    return !!candidate;
  });
  expect(clicked, 'Device Information should be clickable after sign out').toBe(true);
  await waitForText(page, /Device Information|Admin|Reset/i, 20_000);
}

async function clickVisibleControl(page: Page, label: RegExp): Promise<void> {
  const byText = page.getByText(label).first();
  ...
  const clicked = await page.evaluate(({ source, flags }) => {
    const regex = new RegExp(source, flags);
    const candidate = Array.from(document.querySelectorAll<HTMLElement>('ion-button, button, ion-segment-button, ion-tab-button, ion-item, a, div, span, label'))
      .find(el => {
        const visible = !!(el.offsetWidth || el.offsetHeight || el.getClientRects().length);
        const rect = el.getBoundingClientRect();
        if (!visible || rect.width < 10 || rect.height < 10) return false;
        const text = [
          el.innerText,
          el.textContent,
          el.getAttribute('aria-label'),
    ...
    (candidate?.closest('ion-button, button, ion-segment-button, ion-tab-button, ion-item, a') as HTMLElement | null ?? candidate)?.click();
    return !!candidate;
  }, { source: label.source, flags: label.flags || 'i' });
  expect(clicked, `control matching ${label} should be clickable`).toBe(true);
}

async function attemptAdminReset(page: Page): Promise<void> {
  if (await page.locator('body').innerText().then(text => /session open|close it before exiting/i.test(text)).catch(() => false)) {
    await clickOpenSessionContinue(page);
    await navigateToDeviceInformation(page);
  }

  await clickVisibleControl(page, /Admin/i);
  await page.evaluate(() => {
    const tab = Array.from(document.querySelectorAll<HTMLElement>('ion-segment-button, ion-tab-button, ion-col, div, button'))
      .filter(el => {
        const visible = !!(el.offsetWidth || el.offsetHeight || el.getClientRects().length);
        const rect = el.getBoundingClientRect();
        return visible && rect.top < 130 && rect.left > window.innerWidth * 0.55 && /Admin/i.test(el.innerText ?? el.textContent ?? '');
      })
      .sort((a, b) => {
        const aRect = a.getBoundingClientRect();
        const bRect = b.getBoundingClientRect();
  ...
  await page.evaluate(async () => {
    const contents = Array.from(document.querySelectorAll<any>('ion-content'));
  ...
  await waitForText(page, /Reset/i, 10_000);
  await clickVisibleControl(page, /Reset/i);
  await waitForText(page, /unprocessed|pending|transactions|cannot|unable|not allowed|processed/i, 20_000);
}

async function clickOpenSessionContinue(page: Page): Promise<void> {
  await page.locator('.alert-button, ion-alert button, button')
    .filter({ hasText: /^CONTINUE$/i })
    .last()
    .click({ timeout: 5_000, force: true })
    .catch(async () => {
      await page.evaluate(() => {
        const button = Array.from(document.querySelectorAll<HTMLElement>('.alert-button, ion-alert button, button'))
          .find(el => /^CONTINUE$/i.test((el.innerText ?? el.textContent ?? '').trim()));
        button?.click();
      });
    });
  if (await page.locator('body').innerText().then(text => /session open|close it before exiting/i.test(text)).catch(() => false)) {
    await clickVisibleTextByCoordinates(page, /^CONTINUE$/i);
  }
  await expect.poll(
    () => page.locator('body').innerText().then(text => !/session open|close it before exiting/i.test(text)),
    { timeout: 15_000 },
  ).toBe(true);
}
...
test.describe('Transactions', () => {
  test('transactions: search, review flag, details, status, and reset protection', async () => {
    ...
    try {
      ...
      await createPatronSale(window);
      await signOutRapidly(window);
      await navigateToDeviceInformation(window);
      await attemptAdminReset(window);
    } finally {
